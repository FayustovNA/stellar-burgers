...
import { MODALWINDOW_OPEN_ING, MODALWINDOW_CLOSE_ING } from '../../services/action/modal-window';
import { UNSET_INGREDIENTS, SET_INGREDIENTS } from '../../services/action/burger-ingredients';
...
function BurgerIngredients({ }) {
    ...
    const scrollList = () => {
        ...
        setCurrent(prevState => closestTab === prevState.current ? prevState.current : closestTab)
    }
        ...
        <section className={styles.maincontaineringr}>
            ...
            <div className={styles.typeingr} id='ingredientslist' ref={listRef} onScroll={scrollList}>
                <div className={styles.listingr} ref={bunRef}>
                    ...
                    {dataBun.map((card, index) => <CardIngredient key={uuidv4()} counts={countIngredients.get(card._id)} data={card} onClick={(card) => handleOpenModal(card)} />)}
                </div>
                <div className={styles.listingr} ref={sauceRef}>
                    <h2 className={styles.listingrtitle} id='sauce'> Соусы</h2>
                    {dataSauce.map((card, index) => <CardIngredient key={uuidv4()} counts={countIngredients.get(card._id)} data={card} onClick={(card) => handleOpenModal(card)} />)}
                </div>
                <div className={styles.listingr} ref={mainRef}>
                    <h2 className={styles.listingrtitle} id='main'> Начинки</h2>
                    {dataMain.map((card, index) => <CardIngredient key={uuidv4()} counts={countIngredients.get(card._id)} data={card} onClick={(card) => handleOpenModal(card)} />)}
                </div>
